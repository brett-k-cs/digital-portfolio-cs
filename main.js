for(var i = 0; i < document.getElementsByTagName("a").length; i++) {
	...
	if(window.location.href.includes("github")) {
	    ...
	    if(!actualPath.startsWith("/")) actualPath = "/" + actualPath;
...
for(var i = 0; i < dropDowns.length; i++) {
    ...
    const id = element.innerHTML.split("Advanced ")[1]
    if(element) element.onclick = () => {toggleDropdown(id, true)}
    if(mobileCheck()) element.onmouseover = () => {}
}

function toggleDropdown(id, click=false) {
